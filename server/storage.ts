...
export class DatabaseStorage implements IStorage {
  ...
  async seedDemoAccount(): Promise<void> {
    ...
    // Seed document templates and legal norms
  ...
  private async seedDocumentTemplates(): Promise<void> {
    ...
    const existingTemplates = await this.getAllDocumentTemplates();
    if (existingTemplates.length > 0) {
      console.log('✅ Document templates already exist, skipping seeding');
      return;
    }

    console.log('🌱 Seeding document templates...');

    // Template 1: Первичное заявление на удаление данных
    await this.createDocumentTemplate({
      name: 'Первичное заявление на удаление персональных данных',
      documentType: 'initial_request',
      category: 'deletion',
      subject: 'Заявление об удалении персональных данных',
      bodyTemplate: `Кому: {{operatorName}}
От кого: {{userName}}
Email: {{userEmail}}
Телефон: {{userPhone}}
...
На основании Федерального закона от 27.07.2006 № 152-ФЗ "О персональных данных" (статья 14, статья 21) прошу удалить все мои персональные данные, обрабатываемые вашей организацией.
...
Прошу в течение 30 дней с момента получения настоящего заявления:
1. Прекратить обработку моих персональных данных
2. Удалить все персональные данные из ваших баз данных
3. Направить мне письменное подтверждение об удалении данных

В случае отказа в удалении данных прошу предоставить мотивированный ответ со ссылками на законодательство РФ.

Дата: {{currentDate}}
Подпись: {{userName}}`,
      variables: {
        operatorName: 'Название оператора',
        userName: 'ФИО заявителя',
        userEmail: 'Email заявителя',
        userPhone: 'Телефон заявителя',
        birthDate: 'Дата рождения',
        currentDate: 'Текущая дата'
      },
      legalReferences: ['ФЗ-152 ст.14', 'ФЗ-152 ст.21'],
      isActive: true
    });

    // Template 2: Повторное обращение (Follow-up)
    await this.createDocumentTemplate({
      name: 'Повторное обращение по удалению данных',
      documentType: 'follow_up',
      category: 'deletion',
      subject: 'Повторное заявление об удалении персональных данных',
      bodyTemplate: `Кому: {{operatorName}}
От кого: {{userName}}
Email: {{userEmail}}
...
Ссылаясь на мое первичное заявление от {{initialRequestDate}}, которое не было исполнено в установленный законом срок, повторно требую:

В соответствии с ч. 3 ст. 21 Федерального закона от 27.07.2006 № 152-ФЗ "О персональных данных" оператор обязан уведомить субъекта персональных данных об устранении допущенных нарушений или об отказе в таком устранении не позднее чем в течение тридцати дней со дня получения такого требования.

Прошу:
1. Немедленно прекратить обработку моих персональных данных
2. Удалить все данные в течение 7 дней
3. Предоставить письменное подтверждение об удалении

При отсутствии ответа или отказе в удалении данных буду вынужден обратиться в Роскомнадзор и в судебные органы для защиты своих прав.
...
Подпись: {{userName}}`,
      variables: {
        ...
        userName: 'ФИО заявителя',
        userEmail: 'Email заявителя',
        initialRequestDate: 'Дата первичного обращения',
        currentDate: 'Текущая дата'
      },
      legalReferences: ['ФЗ-152 ст.21 ч.3'],
      isActive: true
    });

    // Template 3: Жалоба в Роскомнадзор
    await this.createDocumentTemplate({
      name: 'Жалоба в Роскомнадзор на нарушение ФЗ-152',
      documentType: 'rkn_complaint',
      category: 'complaint',
      subject: 'Жалоба на действия оператора персональных данных',
      bodyTemplate: `В Федеральную службу по надзору в сфере связи,
...
ЖАЛОБА
на действия оператора персональных данных

Оператор: {{operatorName}}
ИНН/ОГРН: {{operatorINN}}
Адрес: {{operatorAddress}}

Суть нарушения:
Оператор персональных данных {{operatorName}} нарушает требования Федерального закона от 27.07.2006 № 152-ФЗ "О персональных данных", а именно:

1. Не удаляет мои персональные данные по моему требованию (нарушение ст. 14, ст. 21 ФЗ-152)
2. Не предоставляет мотивированный ответ на мои обращения (нарушение ст. 21 ч. 3 ФЗ-152)

Хронология событий:
- {{initialRequestDate}} - направлено первичное заявление об удалении данных
- {{followUpDate}} - направлено повторное требование
- На данный момент ответа не получено

Прошу Роскомнадзор:
1. Провести проверку деятельности оператора {{operatorName}}
2. Привлечь оператора к административной ответственности по ст. 13.11 КоАП РФ
3. Обязать оператора удалить мои персональные данные
4. Уведомить меня о результатах проверки

Приложения:
1. Копия первичного заявления от {{initialRequestDate}}
2. Копия повторного обращения от {{followUpDate}}
...
Подпись: {{userName}}`,
      variables: {
        ...
        userPhone: 'Телефон заявителя',
        operatorName: 'Название оператора',
        operatorINN: 'ИНН/ОГРН оператора',
        operatorAddress: 'Адрес оператора',
        initialRequestDate: 'Дата первичного обращения',
        followUpDate: 'Дата повторного обращения',
        currentDate: 'Текущая дата'
      },
      legalReferences: ['ФЗ-152 ст.14', 'ФЗ-152 ст.21', 'КоАП РФ ст.13.11'],
      isActive: true
    });

    console.log('✅ Document templates seeded successfully (3 templates)');
  }
